...
- [x] 1. Mark a match as a title match (revised):
    ...
    - [x] Database:
        - [x] I'll create a new join table called segment_title with segment_id and title_id columns. This will represent the many-to-many relationship between segments and titles.
        - [x] I will need to update the V6__Add_Title_To_Segment.sql migration file to create this table instead of adding a column to the segment table.
    - [x] Backend:
...
- [x] 2. View and update the #1 contender:
    ...
    - [x] Frontend:
        - [x] In the TitleListView, I'll add a ComboBox to each row in the grid to select the #1 contender for that title.
        - [x] The ComboBox will be populated with the wrestlers that are in the same tier as the title.
        - [x] When a new #1 contender is selected, I'll call the new method in the TitleService to update the database.