...
- **Seeded catalog:** `tournaments.json` seeds predefined tournaments at startup (skipped when tournaments already exist, upserted by stable `code` otherwise; re-syncing never touches lifecycle state). Template rows can also reference catalog tournaments directly by picking them in the Tournament combo, or define a spec in the seed catalog itself — the seeded Valentine's Day Massacre PLE (annual, second Sunday of February) carries a full Deadly Combat spec: rounds draw from Last Man Standing / Cage / No DQ, and the final is always a Barbwire Exploding Deathmatch.
