...
- [~] Task: Create `StatusCardService` to manage `StatusCard` entities.
    - [~] Write unit tests for `StatusCardService`.
    - [ ] Implement `StatusCardService`.
- [ ] Task: Create `src/main/resources/status_cards.json` with initial statuses.
- [ ] Task: Update `DataInitializer` to sync status cards from JSON on startup.
    - [ ] Write integration tests for `DataInitializer` sync.
    - [ ] Implement `syncStatusCardsFromFile` in `DataInitializer`.
