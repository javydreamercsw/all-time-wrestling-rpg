...
- [ ] Task: Create `ImageCategory` enum (Wrestler, NPC, Show, Venue, Title, Team, Faction)
    - [ ] Define the enum with category-specific default filenames.
- [ ] Task: Create `ImageSource` interface and `ImageResolution` result object
    - [ ] `ImageSource` should have a method to find an image by name and category.
- [ ] Task: Implement `ClasspathImageSource`
    - [ ] Create TDD unit tests to verify finding images in `src/main/resources/images/`.
    - [ ] Implement the source to search the classpath.
- [ ] Task: Implement `DefaultImageService`
    - [ ] Create TDD unit tests for the resolution logic (specific match -> fallback).
    - [ ] Implement the service to orchestrate multiple `ImageSource`s.
