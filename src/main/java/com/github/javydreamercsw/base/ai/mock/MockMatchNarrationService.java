...
/**
 * Mock implementation of the MatchNarrationService interface. Generates realistic wrestling match
 * narrations without calling external AI services. Perfect for testing, development, and when no AI
 * providers are available.
 */
@Service
@Slf4j
public class MockMatchNarrationService extends AbstractMatchNarrationService {
  ...
  protected String callAIProvider(@NonNull String prompt) {
    ...
    log.info("Mock AI generating match narration (simulated processing time)");
  ...
  /** Generates a realistic mock wrestling match narration. */
  private String generateMockNarration(@NonNull String prompt) {
    ...
    String wrestler1 = wrestlerNames.size() > 0 ? wrestlerNames.get(0) : "Wrestler A";
    String wrestler2 = wrestlerNames.size() > 1 ? wrestlerNames.get(1) : "Wrestler B";

    // Extract venue information
    String venue = extractVenue(prompt);
    String matchType = extractMatchType(prompt);

    StringBuilder narration = new StringBuilder();

    // Opening
    narration.append(generateOpening(wrestler1, wrestler2, venue, matchType));
    narration.append("\n\n");
    ...
    // Mid-match drama
    narration.append(generateMidMatchDrama(wrestler1, wrestler2));
    narration.append("\n\n");
  ...
  private String generateOpening(
      String wrestler1, String wrestler2, String venue, String matchType) {
    String[] openings = {
    ...
    return String.format(opening, venue, matchType, wrestler1, wrestler2);
  }
  ...
  private String generateMidMatchDrama(String wrestler1, String wrestler2) {
    String[] dramas = {
  ...
  private String generateClimaxAndFinish(String wrestler1, String wrestler2) {
    ...
    String[] finishes = {
      ...
      "The crowd is on their feet! %s pulls off the upset of the century! %s fought valiantly but"
          + " %s was just too much tonight! What a match!",
  ...
  /** Extracts match type from the prompt. */
  private String extractMatchType(String prompt) {
    if (prompt.contains("Match Type: ")) {
      String matchTypeSection = prompt.substring(prompt.indexOf("Match Type: ") + 12);
      String matchType = matchTypeSection.split("\n")[0].trim();
      return matchType.isEmpty() ? "wrestling match" : matchType;
    }
