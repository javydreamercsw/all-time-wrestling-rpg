...
/**
 * REST controller for monitoring and managing sync health.
 * Provides endpoints for health checks, metrics, and diagnostics.
 */
@RestController
...
public class SyncHealthController {
  ...
  public ResponseEntity<Map<String, Object>> getStats() {
    try {
      ...
      double failureRate = totalOperations > 0 ? 
          (double) summary.failedSyncs() / totalOperations * 100.0 : 0.0;

      // Recent performance (last 10 operations)
      List<SyncMetric> recentMetrics = metrics.stream()
          .limit(10)
          .toList();
      ...
      double recentSuccessRate = recentMetrics.isEmpty() ? 100.0 :
          (double) recentSuccesses / recentMetrics.size() * 100.0;
      ...
      Map<String, Object> stats = Map.of(
          ...
          "lastFailedSync", summary.lastFailedSync()
      );
  ...
  private List<String> generateRecommendations(SyncHealthSummary summary) {
    ...
    if (summary.consecutiveFailures() >= 3) {
      recommendations.add("Multiple consecutive failures detected. Check NOTION_TOKEN and API connectivity.");
    }

    // Check average sync time
    if (summary.averageSyncTime() > 30000) { // 30 seconds
      recommendations.add("Average sync time is high. Consider optimizing sync operations or checking network performance.");
    }

    // Check stale syncs
    if (summary.lastSuccessfulSync() != null) {
      long hoursSinceLastSuccess = java.time.temporal.ChronoUnit.HOURS.between(
          summary.lastSuccessfulSync(), 
          java.time.LocalDateTime.now()
      );
      
      if (hoursSinceLastSuccess > 24) {
        recommendations.add("No successful sync in over 24 hours. Check sync scheduler and configuration.");
    ...
    if (summary.activeOperations() > 5) {
      recommendations.add("Many active sync operations. Check for stuck operations or reduce sync frequency.");
    }
  ...
  public ResponseEntity<Map<String, Object>> getStatus() {
    try {
      ...
      String status = switch (health.getStatus().getCode()) {
        case "UP" -> "HEALTHY";
        case "DOWN" -> "UNHEALTHY";
      ...
      Map<String, Object> statusResponse = Map.of(
          "status", status,
          "successRate", summary.successRate(),
          "lastSync", summary.lastSuccessfulSync() != null ? 
              summary.lastSuccessfulSync() : "Never",
          "activeOperations", summary.activeOperations(),
          "message", health.getDetails().getOrDefault("error", "OK")
      );
