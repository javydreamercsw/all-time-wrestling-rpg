...
public class BackstageActionView extends VerticalLayout {
  ...
  private void loadCampaign() {
      ...
      Optional<Campaign> c = campaignRepository.findAll().stream().findFirst(); // TODO: Filter by user
  ...
  private void initUI() {
    ...
    buttonLayout.add(createActionButton("Training (Drive)", BackstageActionType.TRAINING, 5)); // Hardcoded attribute for now
    ...
    buttonLayout.add(createActionButton("Attack (Brawl)", BackstageActionType.ATTACK, 5)); // TODO: Heel check

    add(buttonLayout);
    add(new Button("Back to Dashboard", e -> UI.getCurrent().navigate(CampaignDashboardView.class)));
  }
  
  private Button createActionButton(String label, BackstageActionType type, int attributeValue) {
      return new Button(label, e -> {
          BackstageActionService.ActionOutcome outcome = backstageActionService.performAction(currentCampaign, type, attributeValue);
