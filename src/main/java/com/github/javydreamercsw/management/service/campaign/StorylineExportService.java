...
public class StorylineExportService {
  ...
  public String exportStorylineAsChapter(CampaignStoryline storyline) {
    if (storyline == null) {
      throw new IllegalArgumentException("Storyline cannot be null for export.");
    }

    // Determine entry and exit points for the chapter based on the storyline's milestones
    List<CampaignChapterDTO.ChapterPointDTO> entryPoints = List.of(
        CampaignChapterDTO.ChapterPointDTO.builder()
            .name("Storyline: " + storyline.getTitle())
            .criteria(List.of()) // Criteria would need to be inferred or user-defined
            .build()
    );

    List<CampaignChapterDTO.ChapterPointDTO> exitPoints = List.of(
        CampaignChapterDTO.ChapterPointDTO.builder()
    ...
    String aiSystemPrompt = String.format(
        "You are the Campaign Director for the storyline '%s'. Your goal is to guide the player through the narrative described by this storyline.",
        storyline.getTitle()
    );
    ...
    CampaignChapterDTO chapterDTO = CampaignChapterDTO.builder()
        ...
        .difficulty(com.github.javydreamercsw.management.domain.campaign.Difficulty.MEDIUM) // Default to MEDIUM
        ...
        .rules(CampaignChapterDTO.ChapterRules.builder()
  ...
  private String formatId(String title) {
