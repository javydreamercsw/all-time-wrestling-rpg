...
class ShowTypeServiceIT extends ManagementIntegrationTest {
  ...
  void testCreateShowTypeEvictsCache() {
    // First call, should hit the repository
    showTypeService.findAll();
    verify(showTypeRepository, times(1)).findAll();

    // Create a new show type, should evict the cache
    showTypeService.createOrUpdateShowType("Weekly", "Weekly Show", 5, 2);

    // Second call, should hit the repository again
    showTypeService.findAll();
    verify(showTypeRepository, times(2)).findAll();
  }
}
